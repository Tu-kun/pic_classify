...
class pic_classify:
    ...
    def get_keyWords_byseg(self, title, lac):
        ...
        # 摄影师名处理，先判断是否在提供的名单里，若不存在再进行判断
        names = [i for i in d if i in self.names.keys()]
        nationality = [i for i in d if i in self.countries.keys()]  # 摄影师国籍
        loc = [i for i in d if i in self.places]  # 摄影地点

        d_order = sorted(d.items(), key=lambda x: x[1], reverse=True)  # 关键自排序
        top_n = 5
        n = 5 if len(d_order) > top_n else len(d_order)
        key_words = [s[0] for s in d_order[:n] if s[0] not in names]  # 取前5个关键字
    ...
    def get_keyWords_byLac(self, title, lac):
        """
        获取摄影师姓名、国籍、拍摄地
        :param title: 图片标题
        :return: 返回关键字列表
        ...
        # 设置词性过滤规则，只留存关键字
        rule = ['n', 'f', 's', 'vn', 'PER', 'LOC', 'ORG']
        keywords_list = lac.run(title)
        d = {k: v for k, *v in zip(*keywords_list)}  # 将结果转化为字典
        print('切词结果:{}'.format(d))
        d = {k: v for k, v in d.items() if
             k not in self.stop_words.keys() and v[0] in rule and len(k) > 1 and k != '  '}  # 去除停用词,设置词性过滤
        ...
        loc = [i for i in d if i in self.places]
        if len(loc) == 0:
            loc = [i for i in d if d[i] == ['LOC'] and i not in nationality]  # 摄影地点

        print("词性标注结果：{}".format(d))
        return loc, names, nationality
    ...
    def get_data(self, line, lac, seg):
        ...
        loc, name, nationality = self.get_keyWords_byLac(' '.join(title_list[3:]).replace('jpg', ''), lac)
        keyWords = self.get_keyWords(' '.join(title_list[3:]).replace('jpg', ''), seg, name, nationality)
